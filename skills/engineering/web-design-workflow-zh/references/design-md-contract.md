...
`DESIGN.md` 是设计和代码之间的契约。除局部 UI bug 修复外，新网站、新页面、Landing Page、Portfolio、产品页和大改版都应先生成或更新它。快速原型可以精简篇幅，但不能省略关键决策；正式产品要补齐组件状态、内容策略和设计理由。
...
- 图像先行项目要记录视觉稿路径，并把图中的颜色、字体、间距、组件和响应式规则转成文字 token，不能长期靠“看图猜”。

## Lite And Formal

| 模式 | 最低要求 |
|---|---|
| DESIGN.md Lite | 9 个章节均有明确决定；适合单页、Demo、课程或比赛原型 |
| DESIGN.md Formal | 增加完整组件状态、内容语气、素材清单、设计理由、主题/密度变体和验收映射 |
