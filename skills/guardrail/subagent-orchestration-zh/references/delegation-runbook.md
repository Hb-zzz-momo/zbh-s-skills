...
- 一个输出契约。
- 一个停止条件。
- worker 额外要求父代理给出最小任务、唯一写范围、禁止越界规则、验证命令和回收门槛。
...
默认使用只读 explorer/reviewer。父代理明确给出最小任务、唯一写范围、禁止越界规则、验证命令和回收门槛时，可以使用 worker 修改文件；worker 改动必须由父代理复核、集成、测试和最终验收。
...
5. 决定接收、修正、重跑、退回或放弃。
6. 禁止把 worker 的局部完成直接当作最终验收、formal 结果或发布判断。
