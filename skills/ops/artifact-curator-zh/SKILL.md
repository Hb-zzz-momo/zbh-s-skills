...
基于数模等多技能协作项目的归档经验，建议**自本规范起**写入 `skill-outputs` 时采用下列结构（与「文件名里写 `knowledge-digest-zh`」可并存，**目录层级以 skill 名为准**）：
...
│       └─ …
├─ workflow-forge-zh/
│   └─ YYYYMMDD_HHMMSS_workflow-forge-zh_<主题>.md
├─ research-coach-zh/
├─ project-dev-zh/
├─ paper-writing-zh/
└─ …                             # 其它 *-zh / 技能短名
...
- **第二层**：时间戳 + 主题 slug；多文件交付用文件夹，单文件可 `YYYYMMDD_HHMMSS_<skill>_主题.md` 直接放在 skill 目录下（与旧习惯兼容）。  
